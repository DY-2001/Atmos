...
const Charts = ({ projectID, projectInfo, setProjectInfo, userInfoOfUser }) => {
  ...
  async function getTaskInfo(taskId) {
    const res = await fetch(
      `${process.env.REACT_APP_BACKEND_URL}/taskList/${taskId}`
    );
  ...
  const getStats = (userInfo) => {
    ...
    for (let i = 0; i < userInfo.taskAssignedIDList.length; i++) {
      if (
        userInfo.taskAssignedIDList[i].taskDeadline &&
        userInfo.taskAssignedIDList[i].taskCompletion === false &&
        new Date(userInfo.taskAssignedIDList[i].taskDeadline) < Date.now()
      ) {
        count++;
      }
    }
    ...
    for (let i = 0; i < userInfo.taskAssignedIDList.length; i++) {
      if (
        userInfo.taskAssignedIDList[i].taskPriority === "high" &&
        userInfo.taskAssignedIDList[i].taskCompletion === true
      ) {
        count++;
      }
    }
    ...
    for (let i = 0; i < userInfo.taskAssignedIDList.length; i++) {
      if (
        userInfo.taskAssignedIDList[i].taskPriority === "medium" &&
        userInfo.taskAssignedIDList[i].taskCompletion === true
      ) {
        count++;
      }
    ...
    for (let i = 0; i < userInfo.taskAssignedIDList.length; i++) {
      if (
        userInfo.taskAssignedIDList[i].taskPriority === "low" &&
        userInfo.taskAssignedIDList[i].taskCompletion === true
      ) {
        count++;
      }
