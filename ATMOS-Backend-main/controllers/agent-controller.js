    ...
    try {
        ...
        const contextData = {
            ...
            tasks: user.taskAssignedIdList.map(t => ({
                ...
                status: t.taskStatus,
                assignees: t.taskAssigneeList,
                sectionId: t.taskSectionId,
                projectId: t.taskProjectId,
                creatorId: t.taskCreator,
                deadline: t.taskDeadline,
                discussion: t.taskDiscussion,
            ...
            chats: userChats.map(c => ({
                ...
                recentMessages: c.channelMessages.slice(-15).map(m => ({
                    sender: m.userName,
                    content: m.content,
                    time: m.createdAt
                }))
        ...
        const existingHistoryDoc = await AskAgentModel.findOne({ userId: userId }).lean();
        const previousConversations = existingHistoryDoc ? existingHistoryDoc.conversations.slice(-10) : [];

        // Format the history for the LLM
        const historyText = previousConversations.map(turn =>
            `User: ${turn.question}\nAI: ${turn.answer}`
        ).join("\n\n");
